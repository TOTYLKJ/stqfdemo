    ...
    def secure_timespan_verification(self, CT_json, Ts, query_ranges):
        ...
        参数：
            CT_json: 轨迹数据（JSON 格式字符串或已解析的列表/DataFrame），包含字段 
                    'decrypted_traj_id'（轨迹ID，int类型）, 'decrypted_date'（日期，int类型表示天数）, 
                    以及范围标识字段（例如'region_id'）。
            Ts: 时间窗口阈值（整数，表示允许的最大时间跨度长度，单位为天）。
        ...
        # 性能优化：使用pandas的向量化操作
        # 确保decrypted_date是int类型
        CT_df['decrypted_date'] = CT_df['decrypted_date'].astype(int)
        # 确保decrypted_traj_id是int类型
        CT_df['decrypted_traj_id'] = CT_df['decrypted_traj_id'].astype(int)
        # 按轨迹ID和时间排序，确保时间顺序正确
        ...
        for track_id in to_check_tracks:
            ...
            times = track_points['decrypted_date'].to_numpy()         # 日期数组（int）
            ...
            for right in range(len(times)):
                ...
                while have == len(required_set):
                    # 计算当前窗口时间跨度（包含首尾两天）
                    current_span = times[right] - times[left] + 1
                    if current_span <= Ts:
                        found = True
